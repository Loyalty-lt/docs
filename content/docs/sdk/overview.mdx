---
title: "SDK Overview"
description: "Official Loyalty.lt SDKs for Shop API integration"
---
...
<AccordionGroup>
  ...
  <Accordion title="Real-time Updates" icon="bolt">
    Use the Laravel Reverb WebSocket integration (Pusher protocol) for instant notifications when customers scan QR codes. Connection details come from the `realtime/config` endpoint — the SDK's `getRealtimeConfig()` — not a fixed host.
  </Accordion>
</AccordionGroup>
...
Get your credentials from [Partners Portal](https://partners.loyalty.lt) → API Credentials.
