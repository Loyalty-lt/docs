...
```typescript
const coupons = await sdk.listCoupons({ status: 'active' });
```

### Verify & Redeem a Coupon

```typescript
// Verify a coupon code for a shop
const check = await sdk.verifyCoupon('SUMMER10', shopId);

// Redeem it
const result = await sdk.redeemCoupon(couponId, shopId);
```
