...
qr_image_url = f"https://api.qrserver.com/v1/create-qr-code/?size=250x250&data={quote(session['qr_code'])}"
...
class POSSystem:
    ...
    def start_customer_identification(self) -> dict:
        ...
        return {
            'qr_url': f"https://api.qrserver.com/v1/create-qr-code/?size=300x300&data={quote(self.qr_session['qr_code'])}",
    ...
    def redeem_points(self, points: int) -> dict:
        """Redeem customer points."""
        if not self.current_customer:
            raise Exception('No customer identified')
        
        return self.sdk.deduct_points(
            card_id=self.current_customer['id'],
            points=points,
