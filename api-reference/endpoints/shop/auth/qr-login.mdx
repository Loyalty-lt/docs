...
The QR Login system allows users to authenticate on desktop/web by scanning a QR code with their mobile app. This is commonly used in POS systems and kiosk integrations.
...
sequenceDiagram
    ...
    App->>API: POST /auth/qr-login/scan (with qr_code)
    API->>Ably: Publish "scanned" status
    Ably-->>Web: Status update: scanned
    
    App->>API: POST /auth/qr-login/confirm/{session_id}
...
```
POST /{locale}/shop/auth/qr-login/ably-token
```
...
```bash cURL
curl -X POST "https://staging-api.loyalty.lt/en/shop/auth/qr-login/ably-token" \
  -H "Content-Type: application/json" \
  -H "X-API-Key: your_api_key" \
  -H "X-API-Secret: your_api_secret" \
  -d '{
...
const tokenResponse = await fetch('https://staging-api.loyalty.lt/en/shop/auth/qr-login/ably-token', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json',
    'X-API-Key': 'your_api_key',
...
<Note>
This endpoint is used by the **mobile app** (requires JWT authentication) to scan and process QR codes.
</Note>

### Endpoint

```
POST /{locale}/shop/auth/qr-login/scan
```

### Authentication

Requires JWT Bearer token from authenticated mobile app user.

### Request Body

<ParamField body="qr_code" type="string" required>
  The full QR code URL or deep link scanned by the mobile app
  
  **Example:** `loyaltylt://qr-login?code=abc123&session=550e8400-...`
</ParamField>
...
<Steps>
  ...
  <Step title="Receive Tokens">
    Web/POS receives JWT tokens via WebSocket/polling
  </Step>
</Steps>
