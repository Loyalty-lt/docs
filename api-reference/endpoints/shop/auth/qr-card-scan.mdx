...
```
POST /{locale}/shop/qr-card/ably-token
```
...
<ResponseField name="data.channel" type="string">
  Ably channel name: `qr-card:{session_id}`
</ResponseField>

<RequestExample>

```bash cURL
curl -X POST "https://staging-api.loyalty.lt/en/shop/qr-card/ably-token" \
  -H "Content-Type: application/json" \
  -H "X-API-Key: your_api_key" \
  -H "X-API-Secret: your_api_secret" \
  -d '{
...
const tokenResponse = await fetch('https://staging-api.loyalty.lt/en/shop/qr-card/ably-token', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json',
    'X-API-Key': 'your_api_key',
...
const channel = ably.channels.get(`qr-card:${sessionId}`);
...
When subscribing to the Ably channel, you'll receive the following event:
...
class LoyaltyPOS {
  ...
  async startCustomerIdentification() {
    ...
    // 2. Get Ably token
    const tokenResponse = await fetch(`${this.baseUrl}/qr-card/ably-token`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'X-API-Key': this.apiKey,
...
<Note>
This endpoint is called by the Loyalty.lt mobile app when a user scans the QR code. POS integrators don't need to call this endpoint directly.
</Note>

### Endpoint (User API)

```
POST /{locale}/auth/qr-card/scan
```

**Authentication:** JWT Bearer token (mobile app user)

**Request Body:**
```json
{
  "qr_code": "loyaltylt://qr-card?code=abcd1234&session=550e8400..."
}
```

**Response:** Confirms scan success and shows partner/shop info to the mobile app user.
