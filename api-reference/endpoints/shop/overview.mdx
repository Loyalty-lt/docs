...
```bash
curl -X POST "https://api.loyalty.lt/en/shop/transactions/create" \
...
```bash
curl -X GET "https://api.loyalty.lt/en/shop/auth/me" \
...
|--------|----------|------|-------------|
| `POST` | `/auth/request-otp` | None | Request OTP code |
| `POST` | `/auth/login` | None | Login with OTP |
...
| Method | Endpoint | Auth | Description |
|--------|----------|------|-------------|
| `GET` | `/points` | JWT | List point transactions |
| `POST` | `/points/redeem` | JWT | Redeem points |
