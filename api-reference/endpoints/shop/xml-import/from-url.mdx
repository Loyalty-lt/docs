...
const response = await fetch('https://api.loyalty.lt/lt/shop/xml-import/from-url', {
  method: 'POST',
  headers: {
...
```php PHP
<?php
$response = file_get_contents('https://api.loyalty.lt/lt/shop/xml-import/from-url', false, stream_context_create([
    'http' => [
        'method' => 'POST',
        'header' => [
            'Content-Type: application/json',
