...
<Step title="Real-time Authentication">
  Desktop/web receives authentication tokens via Laravel Reverb WebSocket connection.
</Step>
</Steps>
...
<Note>
`session_id` comes from `POST /{locale}/shop/auth/qr-login/generate`. The `qr-login.{session_id}` channel is public — the unguessable session ID is the secret, so no channel authorization is needed.
</Note>
