...
# 2. Get Reverb connection config and subscribe to the qr-login.{sessionId} channel
GET /{locale}/shop/realtime/config

# 3. User scans QR with Loyalty.lt mobile app

# 4. Desktop receives authentication tokens via the Reverb WebSocket (status_update event)
```

Connecting with `pusher-js`:
...
<Card title="User Preferences" icon="cog" href="/api-reference/endpoint/shop-apis#customer-authentication">
