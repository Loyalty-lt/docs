...
- **QR Card Scan**: Instant customer identification at POS terminals
- **Token Management**: JWT refresh and session management
- **Profile Management**: User profile and preferences

**Key Endpoints:**
- `POST /{locale}/shop/auth/qr-login/generate` - Generate QR login session
- `POST /{locale}/shop/qr-card/generate` - Generate QR for POS card identification
...
Customer identification system for POS terminals and kiosks.

**Features:**
- Display QR code on POS screen
- Customer scans with Loyalty.lt mobile app
- Card data returned via WebSocket
- Real-time points balance and redemption settings
- Fallback polling for non-WebSocket environments

**Key Endpoints:**
- `POST /{locale}/shop/qr-card/generate` - Generate QR card scan session
- `GET /{locale}/shop/realtime/config` - Get Reverb WebSocket connection config
- `GET /{locale}/shop/qr-card/status/{sessionId}` - Poll session status

Subscribe to the public `qr-card.{sessionId}` channel and listen for the `card_identified` event:
...
// 3. Subscribe to the public QR card channel
const channel = pusher.subscribe(`qr-card.${session_id}`);

// 4. Listen for the identified card
